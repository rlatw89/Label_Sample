...
class Label: UILabel {
    ...
    var customLineSpacing: CGFloat = lineSpacing
    ...
    private func getAttributedKey(attr: AttributionCase) -> [NSAttributedString.Key : Any] {
        ...
        if let backgroundColor = attr.textBackgroundColor {
            output[NSAttributedString.Key.underlineStyle] = UnderlineLayoutManager.backgroundStyle
        ...
        if let underline = attr.underlineColor {
            output[NSAttributedString.Key.underlineStyle] = (attr == .cbu || attr == .bu) ? UnderlineLayoutManager.underlineStyle : NSUnderlineStyle.single.rawValue
...
extension UIFont {
    ...
    static func customFont(ofSize: CGFloat, weight: UIFont.Weight = .regular) -> UIFont {
        return UIFont.systemFont(ofSize: ofSize + fontIncrement, weight: weight)
    }
